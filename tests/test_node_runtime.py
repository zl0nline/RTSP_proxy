...
from rtsp_proxy.access import PepperVerifier
...
def test_pepper_rotation_requires_helper_reload_before_node_reconfigure() -> None:
    old = PepperVerifier(primary_key_id="old", keys={"old": b"o" * 32})
    overlap = PepperVerifier(
        primary_key_id="new",
        keys={"new": b"n" * 32, "old": b"o" * 32},
    )
    retired = PepperVerifier(primary_key_id="new", keys={"new": b"n" * 32})
    spec = runtime_spec()
    old_config = MediaNodeConfigRenderer(
        auth_callback_port=8010,
        auth_callback_verifier=old,
    ).render(spec, management_credentials(), reader_credentials())
    new_config = MediaNodeConfigRenderer(
        auth_callback_port=8010,
        auth_callback_verifier=overlap,
    ).render(spec, management_credentials(), reader_credentials())
    old_authorization = old.callback_authorization(spec.node_id)
    new_authorization = overlap.callback_authorization(spec.node_id)

    assert old_config.content != new_config.content
    assert "# rtsp-proxy-auth-primary-key-id: old" in old_config.content
    assert "# rtsp-proxy-auth-primary-key-id: new" in new_config.content
    assert overlap.verify_callback_authorization(spec.node_id, old_authorization)
    assert overlap.verify_callback_authorization(spec.node_id, new_authorization)
    assert not retired.verify_callback_authorization(spec.node_id, old_authorization)
    assert retired.verify_callback_authorization(spec.node_id, new_authorization)
...
def test_renderer_produces_one_loopback_managed_ordinary_rtsp_tcp_node() -> None:
    ...
    assert rendered.sha256 == ("9b62e34978fa7f107d5a19cfdc1dc63c9b435c211f2121bc38f8da4a2f12bf63")
...
def test_auth_enabled_smoke_requires_management_and_probe_identities() -> None:
    api = http.server.ThreadingHTTPServer(("127.0.0.1", 0), SmokeHttpHandler)
    metrics = http.server.ThreadingHTTPServer(("127.0.0.1", 0), SmokeHttpHandler)
    rtsp = socket.socket()
    rtsp.bind(("127.0.0.1", 0))
    ...
    pepper = PepperVerifier(primary_key_id="current", keys={"current": b"p" * 32})
    callback_user, callback_password = pepper.callback_credentials(spec.node_id)
    management = management_credentials()
    reader = reader_credentials()
    SmokeHttpHandler.global_config = {
        "authMethod": "http",
        "authHTTPAddress": (
            f"http://{callback_user}:{callback_password}@127.0.0.1:8010"
            f"/internal/v1/media-auth/{spec.node_id}"
        ),
        "authHTTPExclude": [],
        "authInternalUsers": [
            {
    ...
    try:
        MediaNodeSmokeProbe(
            timeout_seconds=1,
            auth_callback_port=8010,
            auth_callback_verifier=pepper,
            reader_credentials=lambda _spec: reader,
        ).check(spec, management)
    finally:
        api.shutdown()
        metrics.shutdown()
        api.server_close()
...
def test_helper_settings_reject_overlap_and_non_absolute_privileged_paths() -> None:
    ...
    assert settings.policy().release_id == "0.2.2"
...
def test_helper_policy_allows_current_and_previous_verified_releases(
    ...
    def trusted_identity(_machine: str, release_id: str = "0.2.2") -> tuple[str, Sha256]:
        digest = TRUSTED_MEDIAMTX_SHA256 if release_id == "0.2.2" else previous_digest
