# Установка, обновление и откат direct-Linux сервера

Эта инструкция описывает одинаковый механизм для первого pilot и production
candidate на direct-Linux сервере. Она устанавливает структуру неизменяемых
релизов и systemd-компоненты, проверяемые в CI, но сама по себе **не выдаёт
допуск к промышленной эксплуатации**. Site admission выполняется по
[`PRODUCTION_RUNBOOK.md`](PRODUCTION_RUNBOOK.md) и текущей
...
`/srv/rtsp-proxy-bundles/0.18.0-amd64`. Не переименовывайте файлы внутри него.
...
sudo --preserve-env=RTSP_PROXY_DEPLOY_UV \
  ...
  --bundle /srv/rtsp-proxy-bundles/0.18.0-amd64
```
...
sudo install -o root -g root -m 0644 \
  /etc/rtsp-proxy/examples/rtsp-proxy.nft.example \
  /etc/rtsp-proxy/rtsp-proxy.nft
```

Если диапазон менялся в env-файлах, сначала тем же образом измените только
`elements` набора `node_ports`. Отсутствие active policy приводит к
`nft_policy_invalid` при запуске `rtsp-proxy-nftables.service`.
...
sudo systemd-run --wait --pipe --collect \
  ...
  /opt/rtsp-proxy/releases/0.18.0/.venv/bin/rtsp-proxy-migrate
sudo -u postgres psql --dbname rtsp_proxy --tuples-only --no-align \
  --command 'SELECT version_num FROM alembic_version;'
```

Вторая команда должна вывести `0026_probe_source_networks`. Отсутствие вывода
...
После первого входа откройте меню **Сменить пароль** и замените временный пароль.
Смена сохраняет текущую сессию, завершает остальные сессии этой учётной записи
и создаёт безопасную audit-запись. Если WEB недоступен, загрузите переменные из
WEB environment file и запустите `rtsp-proxy-local-operator --rotate-password
--username admin`; CLI читает секреты интерактивно и отзывает все web-сессии.

Если аккаунт был создан без `--with-totp`, добавьте MFA без создания второго
администратора:
...
Введите текущий пароль, сохраните показанный URI в authenticator и подтвердите
текущим шестизначным кодом. При успехе все существующие web-сессии аккаунта
отзываются. Команда работает только для аккаунта, у которого TOTP ещё не задан;
она не ослабляет проверку recent MFA.

### 5.1. Разрешённые сети и credentials исходных камер

Политика кандидата `0.18.0`: если камера допускает только одно подключение к
...
максимальный root-owned envelope фактических сетей камер:
...
в форме оператор добавляет адрес источника как `/32` либо его IPv4-подсеть как
`/24`. Эта PostgreSQL-политика аудируется и не может расширить envelope. Режим
аутентификации источника выбирается явно; логин и пароль камеры вводятся в
отдельные поля в исходном виде (не применяйте percent-encoding вручную) и
никогда не возвращаются dashboard.
...
```sh
sudo /opt/rtsp-proxy/releases/0.18.0/.venv/bin/rtsp-proxy-deploy activate \
  --release-id 0.18.0 \
...
Перезапускаются только units, которые были активны до переключения. Update
ожидает HTTPS readiness до 30 секунд. Media nodes
...
Для перехода `0.14.0` → `0.18.0` сначала активируйте новый код на schema 0022,
проверьте smoke, затем выполните migration нового релиза до 0026. Старый manifest
...
бессрочные service grants доступны после migration 0025, а операторская
source-network policy — после migration 0026.

При переходе с `0.17.9`/schema 0025 сначала активируйте `0.18.0` как bridge и
выполните smoke, затем создайте backup и мигрируйте до 0026. Manifest 0.17.9
не допускает schema 0026, поэтому после миграции бинарный rollback на него
...
- обычное FFmpeg-воспроизведение через interleaved TCP для каждого профиля
  камеры;
- success, refusal, timeout и cleanup для source/path probes через отдельные
  worker и root broker;
...
Только после стабильной работы малого контура переходите к node/server ladder
и 24-часовому soak по production runbook. Испытание 100 камер может быть явно
отложено владельцем, но тогда применяется измеренный меньший operational cap и
сохраняется HOLD на полный capacity claim. Ёмкость сервера никогда не выводится
из `max_nodes`.
...
| reconciler/collector сообщает `media_adapter_unavailable` или `media_metrics_unavailable` | установлен release не ниже 0.13.5; проверьте группы каталогов `/run/rtsp-proxy-node-runtime` и `/run/rtsp-proxy-node-metrics` |
| после `sudo install/update` обычный пользователь получает `.git/index: Permission denied` | установлен release не ниже 0.13.6; один раз восстановите владельца checkout и не запускайте Git вручную через `sudo` |
| update исправного сервиса сразу сообщает `activation_health_check_failed_rolled_back` | установлен release не ниже 0.13.7; он ожидает readiness до 30 секунд после restart |
| local admin получает `internal_server_error` при создании ноды | установлен release не ниже 0.13.8; предыдущий audit context не принимал источник личности `local` |
| зарегистрированная нода переходит в `failed`, journal показывает `status=200/CHDIR` | установлен release не ниже 0.13.9; общие runtime/state/log parents должны иметь mode `0751`, а каталоги конкретных нод остаются `0750` |
