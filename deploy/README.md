...
than a cross-file transaction. Candidate `0.16.2` replaces one shared
...
Candidate `0.15.1` retains that schema and tightens the special-address broker
boundary. Metadata/link-local, multicast, unspecified and reserved targets are
not admitted merely because they fall inside a broad configured CIDR. Ordinary
camera admission rejects loopback too; only an explicitly loopback-only broker
...
Candidate `0.16.2` adds schema `0024_camera_probe_profiles`, revision-fenced
...
Candidate `0.17.6` retains schema 0024 and the pinned media/probe binaries. It
...
The intermediate `0.17.5` artifact is also non-admission: its static BPF
program-tag check bound a CO-RE object to the build kernel and rejected the
same verified object after valid relocation on kernel 6.18. Candidate `0.17.6`
uses the cross-kernel identity rule documented in ADR 0004.
