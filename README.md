...
Примеры установки соответствуют кандидату `0.15.3`; используйте только bundle
...
Кандидат `0.15.3` закрепляет пассивный контроль односессионных камер: отдельные
проверки запрещены даже для свободной камеры, чтобы не задерживать зрителя.
Отсутствие свежей глубокой проверки не означает неисправность. Это правило
admission; фоновый worker и его интеграция с Dashboard ещё не включены.
