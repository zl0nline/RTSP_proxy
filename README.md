...
Когда 30-минутное окно recent MFA истекает, локальному оператору не нужно
выходить: защищённая форма показывает обратный отсчёт, принимает текущий
шестизначный TOTP прямо на месте и отправляется без потери уже введённых полей.
Один код нельзя использовать повторно.
...
До первой камеры укажите максимальный разрешённый camera/VLAN CIDR envelope.
Пустой `RTSP_PROXY_PROBE_SOURCE_CIDRS` намеренно запрещает регистрацию любой
камеры и отображается предупреждением в dashboard. Внутри envelope оператор
может при регистрации или смене source явно разрешить только адрес камеры
(`/32`) либо её IPv4-подсеть (`/24`); изменение хранится в PostgreSQL и
аудируется. Подготовить envelope и локальный
...
В форме камеры вводите `rtsp://camera-host/path` без userinfo и явно выберите:
камера требует credentials или заведомо работает без аутентификации. Логин и
пароль вводятся в отдельных полях в исходном виде: не применяйте percent-encoding вручную;
...
Отозванные и истёкшие grants можно удалить одной recent-MFA операцией; каждое
удаление остаётся отдельным нормативным audit event.
